...
if subset:
    if subset_size < 0.02:
        ...
        batch_params['test_batch_size'] = int(512*8)
        
batch_params['train_batch_size'] = int(512*8*3) #int(512*8*4)
batch_params['test_batch_size'] = int(512*8*3)
...
fn_noadv = fn + '_noadv'
if not os.path.isfile(os.path.join(data_path, 'trash', fn_noadv + author +  '_trainer.pickle')):
    mod_noadv = trainer_noadv.train_model(verbose = True)
    io.write_pickled_object(trainer_noadv, os.path.join(data_path, 'trash', fn_noadv + author +  '_trainer.pickle'))
