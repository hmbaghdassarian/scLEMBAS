...
parser.add_argument("bn_weights_lambda_l2", type=float, help="p0")
parser.add_argument("uniform_lambda_L2", type=float, help="p1")
parser.add_argument("cat_max_norm", type=float, help="p2")
parser.add_argument("bn_bias_lambda_L2", type=float, help="p3")

args = parser.parse_args()
bn_weights_lambda_l2 = args.bn_weights_lambda_l2
uniform_lambda_L2 = args.uniform_lambda_L2
cat_max_norm = args.cat_max_norm
bn_bias_lambda_L2 = args.bn_bias_lambda_L2

# usage python script.py param1 param2

# mod_type = 'default' 
# loo = False # leave one out split vs train/test split
...
# cat_max_norm = 100
# bn_bias_lambda_L2 = 0
params_fn = [bn_weights_lambda_l2, uniform_lambda_L2, cat_max_norm, bn_bias_lambda_L2]
...
fn = mod_type + '_' + '_'.join(['p{}_'.format(i) + str(pv).replace('.', '_') for i, pv in enumerate(params_fn)])
...
regularization_params_default = {'input_lambda_L2': 0, # doesn't matter if setting the requires grad to False
                         'bn_weights_lambda_l2': bn_weights_lambda_l2, 
                         'bn_bias_lambda_L2': bn_bias_lambda_L2, # don't incorporate because of KL divergence
                         'output_weights_lambda_L2': 1e-7,
                         'output_bias_lambda_L2': 1e-7,
                         'moa_lambda_L1': 1e2,  
                         'uniform_lambda_L2': uniform_lambda_L2,#, 1e-5,
                         'uniform_min': 0,
                         'uniform_max': 1, 
