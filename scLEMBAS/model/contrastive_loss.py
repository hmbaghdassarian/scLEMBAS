...
class ContrastiveLoss:
    ...
    def __init__(self, 
        ...
        """
        allowed_methods = ['sc_actual', 'sc_predicted', 'sc_triplet', 'bulk_actual', 'bulk_predicted', 'bulk_triplet']
        ...
        self._need_pred_ctrl = any(m in self.methods for m in ['bulk_predicted', 'sc_predicted'])
        self._need_triplet = any(m in self.methods for m in ['bulk_triplet', 'sc_triplet'])
        self._need_bulk_or_triplet = self._need_bulk or self._need_triplet
        self._need_sc = any(m in self.methods for m in ['sc_actual', 'sc_triplet', 'sc_predicted'])
        
        self._need_neg_bulk = any(m in self.methods for m in ['bulk_actual', 'bulk_triplet'])
        self._need_neg_sc = any(m in self.methods for m in ['sc_actual', 'sc_triplet'])
    ...
    def update_calculations(self):
        ...
        for cat, pid in self.combos:
            ...
            if n_ctrl == 0 or n_pert == 0:
                ...
                actual_ctrl_centroid = self.y_out[ctrl_cat_mask].mean(dim=0, keepdim=True)
                actual_pert = self.y_out[cp_mask]
                pred_pert = self.Y_hat[cp_mask]


                # method-specific calculations
                pred_ctrl_centroid = None
                ...
                if self._need_pred_ctrl:
                    pred_ctrl_centroid = self.Y_hat[ctrl_cat_mask].mean(dim=0, keepdim=True).detach()
                if self._need_bulk_or_triplet:
    ...
    - control anchor is the centroid (so not using pairwise distances)
        ...
        for (cat, pid), precalculated in self.combo_cache.items():
            ...
            actual_dist = precalculated['actual_dist_sc']
