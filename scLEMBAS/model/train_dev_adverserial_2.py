       ...
       """
        ...
        for e in trange(self.hyper_params['max_epochs']):
            ...
            for batch, (X_in_, y_out_, covariates_idx_, expr_) in enumerate(self.train_dataloader):
                ...
                discriminator_preds = {} # store to avoid calling forward pass twice, in conjunction with detach notes below
                for cat_group_idx, (cat, discriminator) in enumerate(self.discriminator['discriminators'].items()):
                    discriminator_preds[cat_group_idx] = discriminator(bias_global) # predicted logits
                    # if don't use retain_graph = True, then use bias_global.detach() here
                    # or bias_global_prediction.detach() in the discriminator loss calculation
                    # using it in the discriminator loss calculation has the additional benefit of not having to run
                    # the forward pass of the discriminator 2x 
                     
                discriminator_loss_accuracy = torch.tensor(0, device = self.mod.device, dtype = self.mod.dtype)
                for cat_group_idx, bias_global_prediction in discriminator_preds.items():

                    target = covariates_idx_[:, cat_group_idx]
                    if discriminator.n_labels == 2:
                        target = target.to(self.mod.dtype).unsqueeze(1)

                    discriminator_loss_accuracy += discriminator.loss_fn(bias_global_prediction.detach(), target)   
#                     prediction_loss -= discriminator.loss_fn(bias_global_prediction, target) 
                ...
                # NOTE: since only one forward pass of discriminator, adverserial training lags behind by one batch
