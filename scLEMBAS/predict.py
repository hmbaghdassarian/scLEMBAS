...
def setup_prediction(mod, 
    ...
    binary_pert = True if len(pert_columns) == 2 else False
    if binary_pert:
        non_control_pert = [i for i in pert_columns if i != ctrl_pert][0]
        rev_stim = {ctrl_pert: non_control_pert, non_control_pert: ctrl_pert}
        pert_idx_map = {
            ctrl_pert: 0, 
            non_control_pert: 1
        }
...
def run_prediction(
    ...
    with torch.inference_mode():
        ...
        # edge case check
        zero_idx = [k for k,v in node_idx_map.items() if v == 0][0]
        assert zero_idx not in obs[pert_col], 'Internal: Need to change indexing of control when getting node indices'
        
        node_idx_map[ctrl_pert]=0
        indices = (X_full == mod.input_layer.projection_amplitude).int().argmax(dim=1).detach().cpu().numpy()
        
        if stim_label_map is None:
            ...
            assert all(indices == obs[pert_col].apply(lambda node_name: node_idx_map[stim_label_map[node_name]]).values), 'Input nodes are not being encoded by dummy correctly'
        ################## end sanity check  ##################
...
def project_pls_per_condition(
    ...
    for cond in iter_conds:
        ...
        if umap_models is not None:  
            msg = 'Projecting new data into UMAP space is problematic.\n'
            msg += '1) Calling `.transform` on the previously fitted data reproduces the existing coordinates.' 
            msg += ' However, if projecting new data concatenated with previously fitted data,' 
            msg += '`.transform` produces different coordinates for the previously fitted data.'
            msg += 'It may be better to just `.transform` the new data then concatenate to the existing coordinates.' 
            msg += 'Here, we have applied the first approach (concatenate then call `.transform`.\n'
            msg += '2) See the point here: https://umap-learn.readthedocs.io/en/latest/transform_landmarked_pumap.html'
            warnings.warn(msg)
...
def project_umap_global(tf_adata_actual, tf_adata_merged, only_project_predictions: bool = False):
    """
    Unlike PCA models, umap model's `.transform` changes according to the input batch. 
    
    In other words, previously fitted data will yield the same coordinates only if it is provided
    without new data. Concatenation with new data will change the coordinates of the previously fitted data. 
    
    Here we provide the option to call `.transform` on the entire dataset or just the new data.
    
    See also: https://umap-learn.readthedocs.io/en/latest/transform_landmarked_pumap.html
    
    """
    pc_rank = tf_adata_actual.uns['pca']['pca_rank']
    umap_mod = tf_adata_actual.uns['umap']['umap_mod']
    X_pca = tf_adata_merged.obsm['X_pca'].copy()

    if only_project_predictions:
        actual_mask = (tf_adata_merged.obs.batch == 'actual')
        actual_barcodes = tf_adata_merged[actual_mask, :].obs_names

        X_umap_actual = tf_adata_actual[actual_barcodes, :].obsm['X_umap'].copy()
        X_umap_pred = umap_mod.transform(X_pca[~actual_mask, :pc_rank])

        X_umap = np.empty((tf_adata_merged.n_obs, X_umap_actual.shape[1]), dtype=float)
        X_umap[actual_mask, :] = X_umap_actual
        X_umap[~actual_mask, :] = X_umap_pred
    else:
        X_umap = umap_mod.transform(X_pca[:, :pc_rank])
    return X_umap
